...
if [ "$(mount | grep $DESTINATION)" != "" ]; then
  ...
  while [ "$(qdbus6 org.kde.kwalletd6 /modules/kwalletd6 org.kde.KWallet.isEnabled)" != "true" ]
  do
    $KWALLETD
  done

  #Get the key from KDE Wallet, if nonexisting ask for the key and store it later to KDE Wallet
  WALLETID=$(qdbus6 org.kde.kwalletd6 /modules/kwalletd6 org.kde.KWallet.open kdewallet 0 $APPID)

  PASSWORD=$(qdbus6 org.kde.kwalletd6 /modules/kwalletd6 org.kde.KWallet.readPassword $WALLETID Passwords $DESTINATION $APPID)
  ...
  if [ $? != "" ];
    ...
    if [ $? == "0" ]
      ...
      if [ "$PASSWORD_FETCHED" = "0" ]; then
        A=$(qdbus6 org.kde.kwalletd6 /modules/kwalletd6 org.kde.KWallet.writePassword $WALLETID Passwords $DESTINATION "$PASSWORD" $APPID)
      fi
    ...
    #qdbus6 org.kde.kwalletd6 /modules/kwalletd6 org.kde.KWallet.close $WALLETID false $APPID
